...
var IndexVerified2 = false;
function thingsInit(db){
// json.org

var  thingsColl = db.collection('edificiosUnicah');

var thingsCollection = [];
if ( !IndexVerified) {
  thingsColl.indexExists("nombreEdificio_1", (err, rslt)=>{
    if(!rslt){
      thingsColl.createIndex(
        { "nombreEdificio": 1 },
        { unique: true, name:"nombreEdificio_1"},
        (err, rslt)=>{
          console.log(err);
          console.log(rslt);
      });//createIndex
...
if ( !IndexVerified) {
  thingsColl.indexExists("idEdificio_1", (err, rslt)=>{
    if(!rslt){
      thingsColl.createIndex(
        { "idEdificio": 1 },
        { unique: true, name:"idEdificio_1"},
        (err, rslt)=>{
          console.log(err);
          console.log(rslt);
      });//createIndex
